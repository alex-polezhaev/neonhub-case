...
function buildYandexDescription(product: Product): string {
...
function buildOfferXml(group: ProductGroup, categoryMap: Map<string, number>, siteUrl: string): string | null {
  ...
  const name = `Neon sign ${business.name} ${product.name}`
  const description = buildYandexDescription(product)
  ...
  const lines = [
    ...
    '      <sales_notes>Price and delivery time are confirmed after the order is placed.</sales_notes>',
    ...
    '      <param name="Material">Acrylic base and flexible neon</param>',
